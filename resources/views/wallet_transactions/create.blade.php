...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-md-6">
                    <h1 class="m-0 text-bold">{{trans('lang.wallet_transaction_plural')}}
                        <small class="mx-3">|</small><small>{{trans('lang.wallet_transaction_desc')}}</small></h1>
                </div><!-- /.col -->
                <div class="col-md-6">
                    <ol class="breadcrumb bg-white float-sm-right rounded-pill px-4 py-2 d-none d-md-flex">
                        ...
                        <li class="breadcrumb-item active">{{trans('lang.wallet_transaction_create')}}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
            ...
            <div class="card-header">
                <ul class="nav nav-tabs d-flex flex-row align-items-start card-header-tabs">
                    @can('walletTransactions.index')
                        <li class="nav-item">
                            <a class="nav-link" href="{!! route('walletTransactions.index') !!}"><i class="fas fa-list mr-2"></i>{{trans('lang.wallet_transaction_table')}}
                            </a>
                        </li>
                    @endcan
                    <li class="nav-item">
                        <a class="nav-link active" href="{!! url()->current() !!}"><i class="fas fa-plus mr-2"></i>{{trans('lang.wallet_transaction_create')}}
                        </a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
                {!! Form::open(['route' => 'walletTransactions.store']) !!}
                <div class="row">
                    @include('wallet_transactions.fields')
                </div>
                {!! Form::close() !!}
                <div class="clearfix"></div>
            </div>
