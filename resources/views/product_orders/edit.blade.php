            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb bg-white float-sm-right rounded-pill px-4 py-2 d-none d-md-flex">
                        ...
                        <li class="breadcrumb-item"><a href="{!! route('eserviceBookings.index') !!}">{{trans('lang.eservice_booking_plural')}}</a>
                        </li>
                        <li class="breadcrumb-item active">{{trans('lang.eservice_booking_edit')}}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="card shadow-sm">
            <div class="card-header">
                <ul class="nav nav-tabs d-flex flex-row align-items-start card-header-tabs">
                    @can('eserviceBookings.index')
                        <li class="nav-item">
                            <a class="nav-link" href="{!! route('eserviceBookings.index') !!}"><i class="fas fa-list mr-2"></i>{{trans('lang.eservice_booking_table')}}
                            </a>
                        </li>
                    @endcan
                    @can('eserviceBookings.create')
                        <li class="nav-item">
                            <a class="nav-link" href="{!! route('eserviceBookings.create') !!}"><i class="fas fa-plus mr-2"></i>{{trans('lang.eservice_booking_create')}}
                            </a>
                        </li>
                    @endcan
                    <li class="nav-item">
                        <a class="nav-link active" href="{!! url()->current() !!}"><i class="fas fa-pencil mr-2"></i>{{trans('lang.eservice_booking_edit')}}</a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
                {!! Form::model($eserviceBooking, ['route' => ['eserviceBookings.update', $eserviceBooking->id], 'method' => 'patch']) !!}
                <div class="row">
                    @include('eservice_bookings.fields')
                </div>
                {!! Form::close() !!}
                <div class="clearfix"></div>
            </div>
